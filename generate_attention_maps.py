...
for inputs, labels in test_loader:
    ...
    # this will generate the attention map for a single image. To get avg, save all in list and average.
    #------------------------------------------------------------------------
    print(outputs, attns.shape) 
    # you need to figure out the size of your entire attention matrix and select numbers corresponding to oct, vf, etc.
    curr_attn = torch.mean(attns[0,0,:,:,1:197],1).reshape(12,14,14) # 1:197,197:393,393:589,589:785, mean over all positions, layers
    curr_attn = nn.functional.interpolate(curr_attn.unsqueeze(0), scale_factor=(16,16), mode="bilinear")[0] # scale up the nxn attention map by 16 to original res
    curr_attn = torch.mean(curr_attn,0).cpu().numpy() # mean over all heads
    plt.gcf().set_dpi(300)
    curr_inp = inv_normalize(inputs[0]) # original input image, remove the normalization
    plt.imshow(np.rollaxis(curr_inp[0].cpu().numpy(), 0, 3)) # plot original input image
    plt.imshow(curr_attn, alpha=0.8) # overlay the attention
    plt.savefig("sample_attn_glaucoma")
    # --------------------------------------------------------------------
